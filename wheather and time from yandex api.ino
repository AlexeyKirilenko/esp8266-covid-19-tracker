...
void loop() {
 ...
 tft.begin();
  ...
  tft.setTextSize(2);
  tft.println("Восход в:  "+ SunriseTime); //SunriseTime
  tft.setCursor(20, 220);
  tft.setTextSize(2);
  tft.setTextColor(ILI9341_BLUE);
  tft.println("Закат в:  " + SunsetTime); //SunsetTime
...
bool TimeAndWeather () { 
   ...
   // 
  if (client.connect("yandex.com",443)) {                                   //  conect to remote host (port 443 for https)
    client.print("GET /time/sync.json?geo=" + regionID + " HTTP/1.1\r\nHost: yandex.com\r\nConnection: close\r\n\r\n"); // send request
    delay(200);                                                             // wait for server response
    char endOfHeaders[] = "\r\n\r\n";                                       // parse  responcefrom server
    if (!client.find(endOfHeaders)) {                                       // use only non system part of  responce
      Serial.print("Invalid response");                                   //  check 
      return false;                                                         // stop
    }
    const size_t capacity = 700;                                            // JSON buffer size JSON (https://arduinojson.org/v5/assistant/)
    DynamicJsonBuffer jsonBuffer(capacity);                                 // inisialize JSON buffer
    
    JsonObject& root = jsonBuffer.parseObject(client);                      // parse JSON answer
    client.stop();     // Разрываем соединение с сервером
    Serial.print("client stop"); 
    String StringCurrentTime = root["time"].as<String>().substring(0,10);   // get local date time witout ms
    String StringOffset =  root["clocks"][regionID]["offset"].as<String>(); // use time shift in ms
    SunriseTime =  root["clocks"][regionID]["sunrise"].as<String>();        // sunrise  time   clocks ->  RegionID -> sunrise 
    SunsetTime =  root["clocks"][regionID]["sunset"].as<String>();          // sunset time    clocks ->  RegionID -> sunset
    Temperature =  root["clocks"][regionID]["weather"]["temp"].as<String>();//   clocks ->   RegionID -> weather -> temp
    strcpy(icon, root["clocks"][regionID]["weather"]["icon"].as<String>().c_str());       //  clocks -> значение RegionID -> weather -> icon

    jsonBuffer.clear();                                                     // clear buffer JSON
    
    unsigned long CurrentTime = StringToULong(StringCurrentTime);           //   String в unsigned long
    unsigned long Offset = StringToULong(StringOffset) / 1000;              //   String в unsigned long  and use hours,minutes, seconds
    setTime(CurrentTime + Offset);                                          // synctime
...
unsigned long StringToULong(String Str) {                     // convert String to unsigned long
  unsigned long ULong = 0;
  for (int i = 0; i < Str.length(); i++) {                    //  convert string to unsigned long in circle
     char c = Str.charAt(i);
     if (c < '0' || c > '9') break;
     ULong *= 10;
     ULong += (c - '0');
...
void digitalClockDisplay(){                                   // print date+time on serial port
